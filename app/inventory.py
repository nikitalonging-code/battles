...
async def sync_bank_gifts(session: AsyncSession, bot: Bot) -> int:
    ...
    Возвращает количество новых зачисленных подарков.
    """
    ...
    # Уже известные owned_gift_id (включая выведенные — чтобы не дублировать)
    ...
    for owned in result.gifts:
        ...
        if sender is None:
            # Неопознанный отправитель — пропускаем (или можно логировать)
            logger.info(
                "sync_bank_gifts: gift %s without sender_user, skip",
                owned.owned_gift_id,
            )
            continue
...
async def withdraw_item(
    ...
    conn_id = settings.bank_business_connection_id
    if not conn_id:
        raise InventoryError("Банк подарков не настроен (BANK_BUSINESS_CONNECTION_ID)")
